...
local function updateSlider(sliderDrag, val, minvalue, maxvalue, mousePosition)
    Value = math.floor((((tonumber(maxvalue) - tonumber(minvalue)) / 149) * sliderDrag.AbsoluteSize.X) + tonumber(minvalue))
    val.Text = Value
    sliderDrag:TweenSize(
        UDim2.new(0, math.clamp(mousePosition - sliderDrag.AbsolutePosition.X, 0, 149), 0, 6),
...
sliderBtn.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        sliding = true
        TweenService:Create(val, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {
            TextTransparency = 0
        }):Play()
